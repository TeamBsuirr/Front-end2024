...
export default function SearchResultPage() {
    ...
    useEffect(() => {
        ...
        searchService.getGlobalSearch(urlParamsString)
            .then(data => {
                //console.log(data);
                ...
                const humansFormatted = data.humans.map(human => ({
                    id:human?.id,
                    type: "humans",
                    img: human.images && human.images.length > 0 ? human.images[0].urlToFile : "",
                    header: `${human.surname} ${human.name} ${human.patronymic}`,
                    description: human.history?.description.split(' ').slice(0, 15).join(' ')
                }));
                ...
                const placesFormatted = data.places.map(place => ({
                    id:place?.id,
                    type: "places",
                    img: place.images && place.images.length > 0 ? place.images[0].urlToFile : "",
    ...
    } else if ((humans === null && places === null) || (!humans && !places) || searchString === "") {
        return <NotFound />;
    } else {
        return (
            <SearchResults arrayFoundObjects={arrayFoundObjects} />
        );
    }
