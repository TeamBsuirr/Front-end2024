...
const searchService = {
         ...
         response.data.humans.content.map(item => addMainImagePreview(item)); // Применяем функцию к каждому объекту массива
            ...
            return {
                data: {
                    ...
                    humans: response.data.humans.content.map(item => addMainImagePreview(item)),
                    places: response.data.places.content.map(item => addMainImagePreview(item)),
    ...
    postPhoto: (data) => {
        ...
        return handleRequest(() =>
            api.post("/albums", transformedData, {
                headers: {
    ...
    updatePhoto: (data) => {
        ...
        // Выполняем запрос, добавляя заголовок Content-Type
        return handleRequest(() =>
            api.put(`/albums/${data.id}`, transformedData, {
                headers: {
                    "Content-Type": "multipart/form-data",
                },
            }),
